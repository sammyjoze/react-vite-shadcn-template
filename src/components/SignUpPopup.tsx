...
const SignUpPopup: React.FC<SignUpPopupProps> = ({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await signup({
        ...
        password: 'defaultPassword123', // Mock password for simplified form
        username: formData.username,
      });
      
      // Close popup on successful signup
      onClose();
    } catch (error) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-[95vw] max-w-md mx-auto p-0 sm:p-6">
        ...
        <Card className="border-0 shadow-none">
          <CardContent className="p-4 sm:p-0">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Button
                type="submit"
                className="w-full h-11 sm:h-10 bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70 transition-all duration-200 shadow-lg hover:shadow-xl"
                disabled={isLoading}
              >
                {isLoading ? (
                  <>
            ...
            <div className="mt-6">
              ...
              <div className="mt-4 text-center text-sm text-muted-foreground px-2">
                ...
                <Button 
                  variant="link" 
                  className="p-0 h-auto font-normal hover:text-primary transition-colors duration-200"
              ...
              <div className="mt-4 text-center text-xs text-muted-foreground px-2">
                ...
                <Button 
                  variant="link" 
                  className="p-0 h-auto font-normal text-xs hover:text-primary transition-colors duration-200"
                  onClick={onOpenTerms}
                >
                  Terms
                </Button>{' '}
